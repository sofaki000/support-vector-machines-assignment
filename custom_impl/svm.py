...
class SVM():
    ...
    def fit(self, data, labels):
        ...
        K_table = self.transform(data)
        P = cvxopt.matrix(np.outer(labels, labels) * K_table)
        q = cvxopt.matrix(np.ones(num_data) * -1)
        A = cvxopt.matrix(labels, (1, num_data))
        b = cvxopt.matrix(0.0)
        # creates a diagonal table with ones in diagonal and zeros, num_data * num_data
        ...
        if self.kernel == "linear":
            ...
            for i in range(X.shape[0]):
                ...
                for alpha, label, sv in zip(self._alphas, self._support_labels, self._support_vectors):
                    kernel_num_of_sv_and_test_sample = self.kf[self.kernel](test_sample, sv)

                    s = s + alpha * label *  kernel_num_of_sv_and_test_sample

                # to kathe sample exei ena score sto opoio tha prosthesoume
                # to b kai tha doume an einai
                # thetiko h arnhtiko
                score[i] = s
